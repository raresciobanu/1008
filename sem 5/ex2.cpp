...
void sortareFisierInMemorieInternaDupaDenumireSiPret(char numeFisier[20]) {
	...
	if (f != NULL) {
		...
		Produs aux;
		Produs* produse = (Produs*)malloc(sizeof(Produs) * nr);
		fread(produse, sizeof(Produs), nr, f);//scriem in vectorul dinamic de produse toate elementele din fisier.

		if (produse != NULL) {
			for (int i = 0; i < nr - 1; i++) {
				for (int j = i + 1; j < nr; j++)
				{
					if (strcmp(produse[i].denumire, produse[j].denumire) > 0 ||
						(strcmp(produse[i].denumire, produse[j].denumire) == 0 && produse[i].pret > produse[j].pret)) {
						aux = produse[i];
						produse[i] = produse[j];
						produse[j] = aux;
					}
			...
			rewind(f);
			fwrite(produse, sizeof(Produs), nr, f);//suprascrii tot fisierul cu vectorul tau
		}
